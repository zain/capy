...
export function PublicAnalytics({ event }: { event?: string }) {
  ...
  useEffect(() => {
    ...
    if (event) window.posthog?.capture(event);
  }, [event]);
  return null;
}
